/**
 * args/fs.c - Implementation of filesystem syscall parsers.
 */
...
{
        FMT_BYTES_MEM_IF_OK(ctx, args[1], ctx->retval);
        FMT_SEPARATOR(ctx);
        FMT_INT(ctx, args[2]);
...
DEFINE_SYSCALL_ENTER_PARSER(write)
{
        FMT_FD(ctx, args[0]);
        FMT_SEPARATOR(ctx);
        FMT_BYTES_MEM(ctx, args[1], args[2]);
        FMT_SEPARATOR(ctx);
        FMT_INT(ctx, args[2]);
...
{
        FMT_BYTES_MEM_IF_OK(ctx, args[2], ctx->retval);
        FMT_SEPARATOR(ctx);
        FMT_INT(ctx, args[3]);
...
{
        FMT_BYTES_MEM_IF_OK(ctx, args[0], ctx->retval);
        FMT_SEPARATOR(ctx);
        FMT_INT(ctx, args[1]);
...
DEFINE_SYSCALL_ENTER_PARSER(setxattr)
{
        ...
        FMT_CSTRING_MEM(ctx, args[1]);
        FMT_SEPARATOR(ctx);
        FMT_BYTES_MEM(ctx, args[2], args[3]);
        FMT_SEPARATOR(ctx);
        FMT_LLU(ctx, args[3]);
        FMT_SEPARATOR(ctx);
